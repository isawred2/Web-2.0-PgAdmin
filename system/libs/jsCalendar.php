...
function jsCalendar_class(name, box) {
	// variables
...
top.jsCalendar = jsCalendar_class;
...
function jsCalendar_getMonth(year, month) {
    ...
    html  = '<table cellpadding="3" cellspacing="0" style="background-color: #e4ebed; border: 1px solid silver;" class="rText">'+
    	    '	<tr><td colspan="7" align="center" style="padding: 4px; border-bottom: 1px solid silver; background-image: url(/system/images/bl_title_bg.png); color: black"> '+ this.months[month-1] +', '+ year +' </td></tr>'+
    	    '	<tr><td '+cc+'>M</td> <td '+cc+'>T</td> <td '+cc+'>W</td>'+
    	    '	    <td '+cc+'>T</td> <td '+cc+'>F</td> <td '+cc+'>S</td> <td '+cc+'>S</td>'+
    	    '	</tr>'+
    	    '	<tr>';
    for (ci=1; ci<43; ci++) {
    	if (ci < weekDay || (ci-weekDay+1) > this.daysCount[month-1]) {
			html += '<td style="cursor: default; background-color: #f4f4fe; border: 1px solid #e8e8e8;">&nbsp;</td>';
	    	if ((ci)%7 == 0) html += '</tr><tr>';
    		continue;
    	}
    	dt = month + '/' + (ci-weekDay+1) + '/' + year;
    	if (dt == this.curDate) bor = 'border: 1px solid #c8493b;'; else bor = 'border: 1px solid #e8e8e8;';
    	if ((ci+0)%7 == 0)  col = 'color: #c8493b;'; else col = 'color: black;'; 
    	dspDay = (ci-weekDay+1);
...
function jsCalendar_getHours(id) {
	var el  = this.ownerDocument.getElementById(this.name + '_tbl');
...
// Class is Loaded
if (top.jsLoader) top.jsLoader.loadFileDone('jsCalendar.js');