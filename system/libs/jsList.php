...
function jsListItem_class(id, values) {
	this.id    		= id;
    this.values		= values;
    this.selected 	= false;
...
function jsList_class(name, box) {
	// public properties
    this.name  	  		= name;
...
top.jsListItem   = jsListItem_class;
top.jsList   	 = jsList_class;
	...
	switch (String(type).toLowerCase()) {
    	case 'add':
        	html = '<div id="'+ this.name +'_control'+ ind + '_div">'+
				   ...
				   '<td style="padding-left:3px;"><img src="'+ top.jsLoader.sys_path +'/includes/silk/icons/add.png"></td>'+
				   '<td nowrap style="padding: 2px; padding-bottom: 1px; padding-left: 5px"><a style="padding-right: 5px; cursor: pointer; font-weight: bold" id="'+ this.name +'_control'+ ind + '" class="rText" onclick="obj = top.elements[\''+ this.name +'\'].onAddOrEdit; if (typeof(obj) == \'function\') { obj(); } else if (typeof(obj) == \'object\') { obj.box = top.elements[\''+ this.name +'\'].box; obj.recid = null; obj.output(); } else { top.elements[\''+ this.name +'\'].serverCall(obj.onAddOrEdit); }" '+ param +'>'+ caption + '</a></td>'+
				   '</tr></table></div>';
            break;
    	case 'delete':
        	html = '<div id="'+ this.name +'_control'+ ind + '_div">'+
				   '<table cellpadding=0 cellspacing=0 class="lstButton"><tr>'+
				   '<td style="padding-left:3px;"><img src="'+ top.jsLoader.sys_path +'/includes/silk/icons/delete.png"></td>'+
        ...
        case 'save':
        	html = '<div id="'+ this.name +'_control'+ ind + '_div">'+
				   '<table cellpadding=0 cellspacing=0 class="lstButton"><tr>'+
				   '<td style="padding-left:3px;"><img src="'+ top.jsLoader.sys_path +'/includes/silk/icons/accept.png"></td>'+
...
function jsList_addSearch(caption, type, fieldName, inTag, outTag, defValue, items) {
    ...
    if (type.toUpperCase() != 'BREAK') {
	    ...
	    this.searches[ind].inTag   = ' onkeyup="if (event.keyCode == 13) { obj = top.elements[\''+ this.name +'\']; if (obj) { obj.openSearch(false); obj.submitSearch() } }" '+ this.searches[ind].inTag;
    }
...
function jsList_openSearch(flag) {
    ...
    if (this.showSearch) {
		...
		// focus first element
    	sel = this.box.ownerDocument.getElementById(this.name + '_field' + this.searches[0].index);
    	if (sel) { sel.focus(); }
    }
}
...
function jsList_resize() {
	...
	if (height > 0 && el) {
		el.style.height = height - (this.showHeader ? 27 : 0) - (this.showFooter ? 25 : 0) - (this.toolbar != null ? 30 : 0) - (document.all ? 0 : 4);
	}
	if (width > 0 && el) {
		el.style.width = width - (document.all ? 0 : 4);
	}
}
...
function jsList_refresh() {
    ...
    if (this.layout == 'table') {
       bodyHTML += '<table cellpadding="0" cellspacing="0" style="width: 100%" class="lstBody_tbl">\n'+
                        this.getHeaders() +
                        this.getRecords() +
                    '</table>\n';
    }
    if (this.layout == 'div') {
       bodyHTML += '<table style="width: 100%" class="lstBody_tbl"><tr><td>\n'+
                        this.tmpl_start +
                        this.getHeaders() +
                        this.getRecords() +
                        this.tmpl_end +
    ...
    if (this.box.ownerDocument.getElementById('body_'+ this.name)) {
	    ...
	    if (this.showFooter) {
		    ...
		    if (el) el.innerHTML = pageCountDsp;
	    }
	    // refresh body
        this.box.ownerDocument.getElementById('body_'+ this.name).innerHTML = bodyHTML;
    	...
    	// output entire thing
	    html =  '<div id="list_'+ this.name +'" class="lst_div">';
		// generate header
	    if (this.showHeader) {
		    html += '<div id="header_'+ this.name +'" class="lstHeader_div" style="height: 26px; overflow: hidden;">\n'+
					...
					'		 <span ondblclick="top.elements.'+ this.name +'.getData()" id="title_'+ this.name +'">'+ this.header +'&nbsp;</span>'+
		                     ( this.searches.length != 0 ? ' - <span class="rText">[ <a href="javascript:top.elements[\''+ this.name +'\'].openSearch();">Search</a> ]</span>' : '') +
	    ...
	    html += '<div id="body_'+ this.name +'" class="lstBody_div" style="height: 250px; overflow: auto;">\n';

		html += bodyHTML + '</div>\n';
	    // generate footer
	    if (this.showFooter) {
	    	html += '<div id="footer_'+ this.name +'" class="lstFooter_div" style="height: 24px; overflow: hidden;">\n'+
		            '   <table cellpadding=0 cellspacing=1 style="height: 24px; width: 100%" class="lstFooter_tbl"><tr>\n'+
		            '       <td id="footerL_'+ this.name +'" style="width: 80%" nowrap="nowrap" class="lstFooter_td1">'+ pages +'</td>\n'+
		            '       <td id="footerR_'+ this.name +'" style="width: 20%" nowrap="nowrap" align=right class="lstFooter_td2">'+ pageCountDsp +'</td>\n'+
		            '   </tr></table>'+
		            '</div>';
	    }
...
function jsList_getSearches() {
	html = '';
	html = '<table cellspacing="0" cellpadding="1" class="lstSearch_tbl">';
    for (i=0; i<this.searches.length; i++) {
    	btn = '';
    ...
    html += '<tr>'+
            ...
            '		<input type="button" onclick="obj = top.elements[\''+ this.name +'\']; if (obj) { obj.openSearch(false); obj.submitSearch(); }" style="width: 70px" class="rButton" value="Search">'+
            '		<input type="button" onclick="obj = top.elements[\''+ this.name +'\']; if (obj) { obj.openSearch(false); obj.clearSearch(); obj.submitSearch(); }" style="width: 70px" class="rButton" value="Clear">'+
            '	</td>'+
    		'</tr></table>';
    return html;
}
    ...
    if (istart > 8) {
    	...
    	if (this.smallPageNav) {
	    	pagesa = "<div style=\"float: left; padding-left: 15px;\">"+
	                 ...
	                 "  		onclick=\"if (top.elements[\'"+ this.name +"\'].page_num > 0) { top.elements[\'"+ this.name +"\'].items = []; top.elements[\'"+ this.name +"\'].refresh(); "+ this.name +".page_num -= 1; "+ this.name +".getData(); }\" "+
	    			 "		>"+
	    			 "		<input type=\"button\" value=\">>\" class=\"sText\" style=\"width: 30px\" "+
	                 "  		onclick=\"if (top.elements[\'"+ this.name +"\'].page_num < top.elements[\'"+ this.name +"\'.page_count) { top.elements[\'"+ this.name +"\'].items = []; top.elements[\'"+ this.name +"\'].refresh(); top.elements[\'"+ this.name +"\'].page_num += 1; top.elements[\'"+ this.name +"\'].getData(); }\" "+
	    			 "		>"+
	    			 "</div>";
    	} else {
	    	pagesa = "<div style=\"float: left; padding-left: 15px;\">"+
	    			 "		<input type=\"button\" value=\"<<\" class=\"sText\" style=\"width: 30px\" "+
	                 "  		onclick=\"if (top.elements[\'"+ this.name +"\'].page_num > 0) { top.elements[\'"+ this.name +"\'].items = []; top.elements[\'"+ this.name +"\'].refresh(); top.elements[\'"+ this.name +"\'].page_num -= 1; top.elements[\'"+ this.name +"\'].getData(); }\" "+
	    			 "		>"+
	    			 ...
	    			 "					  if (this.value > top.elements[\'"+ this.name +"\'].page_count) this.value = top.elements[\'"+ this.name +"\'].page_count; "+
	    			 "					  top.elements[\'"+ this.name +"\'].items = []; "+
	    			 "					  top.elements[\'"+ this.name +"\'].page_num = 0; "+
	    			 "					  top.elements[\'"+ this.name +"\'].refresh(); "+
	    			 "					  top.elements[\'"+ this.name + "\'].page_num = parseInt(this.value-1); "+
	    			 "					  top.elements[\'"+ this.name +"\'].getData(); \">"+
	    			 "		<input type=\"button\" value=\">>\" class=\"sText\" style=\"width: 30px\" "+
	                 "  		onclick=\"if (top.elements[\'"+ this.name +"\'].page_num < top.elements[\'"+ this.name +"\'].page_count) { top.elements[\'"+ this.name +"\'].items = []; top.elements[\'"+ this.name +"\'].refresh(); top.elements[\'"+ this.name +"\'].page_num += 1; top.elements[\'"+ this.name +"\'].getData(); }\" "+
	    			 "		>"+
	    			 "</div>";
    ...
    if (!this.smallPageNav) {
	    for (i=1; i<=istart; i++) {
	        ...
	        pages += '<div class="'+ cName +'" style="float: left; cursor: pointer;" '+
	                 ...
	                 '  onclick = "top.elements[\''+ this.name +'\'].items = []; top.elements[\''+ this.name +'\'].refresh(); top.elements[\''+ this.name +'\'].page_num = '+ (i-1) +'; top.elements[\''+ this.name +'\'].getData();" '+
	                 '>' + i + '</div>';
	    }
	} else {
        	...
        	if (this.showTabHeader) {
	            ...
	            for (i=0; i<this.columns.length; i++) {
					...
					if (this.sortBy[i] && this.sortBy[i] != '') {
			    		...
			    		sortStyle = 'background-image: url('+ top.jsLoader.sys_path +'/images/'+ img +'); background-repeat: no-repeat; background-position: center right; ';
			    	} else {
			    		sortStyle = '';
			    	}
                    	...
                    	switch (tmp[0].toUpperCase()) {
                    		case 'INT':
		                    	html +=	'<input id="'+ this.name +'_edit_'+ i +'_'+ j +'" class="rText" type="text" '+
		                    			'	style="border: 1px solid #d2cedf; width: 100%; padding: '+ (this.tblPadding-2) +'; '+ (tmp[1] != undefined ? tmp[1] : '') +'" '+
		                    			...
		                    			'	onblur="if (!top.jsUtils.isInt(this.value)) { this.value = this.getAttribute(\'oldvalue\'); } if (this.getAttribute(\'oldvalue\') != this.value) { this.style.backgroundColor = \'#ffffb2\'; } else { this.style.backgroundColor = \'white\'; }"'+
		                    			'>';
                    			break;
                    		case 'FLOAT':
		                    	html +=	'<input id="'+ this.name +'_edit_'+ i +'_'+ j +'" class="rText" type="text" '+
		                    			'	style="border: 1px solid #d2cedf; text-align: right; width: 100%; padding: '+ (this.tblPadding-2) +'; '+ (tmp[1] != undefined ? tmp[1] : '') +'" '+
		                    			'	oldvalue="'+ printItem +'" value="'+ printItem +'" '+
		                    			'	onclick="this.select();"'+
		                    			'	onkeyup="if (event.keyCode == 40 || event.keyCode == 13) { el = document.getElementById(\''+ this.name +'_edit_'+ (i+1) +'_'+ j +'\'); if (el) {el.select(); el.focus(); } } '+
		                    			'			 if (event.keyCode == 38) { el = document.getElementById(\''+ this.name +'_edit_'+ (i-1) +'_'+ j +'\'); if (el) {el.select(); el.focus(); } } "'+
		                    			'	onfocus="obj = top.elements[\''+ this.name +'\']; obj.lstClick('+ i +', event);"'+
		                    			'	onblur="if (!top.jsUtils.isFloat(this.value)) { this.value = this.getAttribute(\'oldvalue\'); } if (this.getAttribute(\'oldvalue\') != this.value) { this.style.backgroundColor = \'#ffffb2\'; } else { this.style.backgroundColor = \'white\'; }"'+
		                    			'';
                    			break;
                    		default:
		                    	html +=	'<div title="'+ printItem +'" style="overflow: hidden; '+ this.divStyle + '">' +
		                        			printItem +
		                        		'</div>';
		                        break;
                    	}
                    } else {
                    	html +=	'<div style="padding: '+ this.tblPadding +'; '+ this.divStyle +'">' + printItem + '</div>';
                    }
                    html += '</td>';
                	j++;
...
function jsList_columnClick(ind, evnt) {
	if (this.onSort) {
    	fl = this.onClick(parseInt(ind) + 1, evnt);
        if (fl === false) return;
    }
...
function jsList_getData() {
    ...
    for (si=0; si<this.searches.length; si++) {
    	...
    	if (sel) { tmpSearchData[this.searches[si].fieldName] = sel.value + el2dsp; }
    	    else { tmpSearchData[this.searches[si].fieldName] = (this.searches[si].value != null && this.searches[si].value != '') ? this.searches[si].value : this.searches[si].defValue; }
    	this.searches[si].value = tmpSearchData[this.searches[si].fieldName];
    }
    if (!searchFlag) tmpSearchData = '';
    // add list params
...
// Class is Loaded
if (top.jsLoader) top.jsLoader.loadFileDone('jsList.js');