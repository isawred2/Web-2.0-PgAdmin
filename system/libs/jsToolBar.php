...
function jsToolBarItem_class(id, owner, type, caption, picture) {
    this.id       = id;
    this.owner    = owner;
...
function jsToolBarLink_class(level, caption, url, target) {
	this.level    = level;
	this.caption  = caption;
	this.url	  = url;
	this.target   = target;
...
function jsToolBar_class(name, box) {
    // required
    this.name      = name;
    this.box	   = box;
    this.items     = [];
...
top.jsToolBarLink = jsToolBarLink_class;
top.jsToolBarItem = jsToolBarItem_class;
top.jsToolBar     = jsToolBar_class;
...
function jsToolBarItem_getHTML() {
    ...
    if (document.all) transparent = this.owner.bgcolor; else transparent = 'transparent';
    addToText = '';
    if (this.picture != '') {
	    if (document.all) {
	    	if (this.disabled) {
		        ...
		        butPicture  = 'src="'+ top.jsLoader.sys_path +'/images/empty.gif" style="Filter:Progid:DXImageTransform.Microsoft.AlphaImageLoader(src='+ this.picture + ', opacity=20)"';
	    	}
	    } else {
    ...
    switch (this.type) {
        case 'button':
            html +=  '<table cellpadding=0 cellspacing=0 title="'+ this.hint +'" id="tab0_'+ this.owner.name + '_' + this.id +'"'+
                     ...
                     '       style="-moz-border-radius: 4px; -webkit-border-radius: 4px; height: 22px; margin-top: 2px; border: 1px solid '+ transparent +';"><tr><td>'+
                     '  <table cellpadding=2 cellspacing=0 id="tab1_'+ this.owner.name + '_' + this.id +'"'+
                     '       style="-moz-border-radius: 4px; -webkit-border-radius: 4px; height: 100%; border: 1px solid '+ transparent +'; '+
        ...
        case 'check':
            ...
            html +=  '<table cellpadding=0 cellspacing=0 title="'+ this.hint +'" id="tab0_'+ this.owner.name + '_' + this.id +'"'+
                     ...
                     '       onmousedown = "var el=top.elements[\''+ this.owner.name + '\']; if (el) el.itemDown(\''+ this.id +'\');" '+
                     '       onmouseup   = "var el=top.elements[\''+ this.owner.name + '\']; if (el) el.itemUp(\''+ this.id +'\');" '+
                     '       style="-moz-border-radius: 4px; -webkit-border-radius: 4px; height: 22px; margin-top: 2px; '+ addCheckStyle +';"><tr><td>'+
                     '  <table cellpadding=2 cellspacing=0 id="tab1_'+ this.owner.name + '_' + this.id +'"'+
                     '       style="-moz-border-radius: 4px; -webkit-border-radius: 4px; height: 100%; border: 1px solid '+ transparent +'; '+ addCheckStyle2 +
                     '              border-left: 0px solid '+ transparent +'; border-top: 0px solid '+ transparent +'; cursor: default;"><tr>'+
                        (this.picture != '' ? '<td><img '+ butPicture +'></td>' : '') +
                        (this.caption != '' ? '<td nowrap style="font-size: 11px; font-family: verdana; '+ addToText +'">'+ this.caption +'</td>' : '') +
                     '  </tr></table>'+
        ...
        case 'html':
            html +=  '<table cellpadding=2 cellspacing=0 id="tab_'+ this.owner.name +'_'+ this.id +'" style="height: 22px; margin-top: 2px;'+ addToText +'"><tr>'+
                     '    <td>' + this.caption + '</td>'+
                     '</tr></table>';
            break;
    }
...
function jsToolBarItem_refresh() { this.owner.refresh(this.id); }
...
function jsToolBar_output() {
	...
	html  = '<table cellspacing="0" cellpadding="1" style=""'+
    		...
    		'		onmousedown = "if (document.all) document.selection.empty(); else window.getSelection().removeAllRanges();"'+
    ...
    for (var i=0; i<this.items.length; i++) {
        ...
        if (this.items[i].disabled) { addStyle += '-moz-opacity: 0.2;'; } 
        html += '<td id="item_'+ this.items[i].id +'" style="'+ addStyle +'" valign=middle>'+ this.items[i].getHTML() + '</td>';
    }
    html += '<td width="100%" id="'+ this.name +'_right"></td></tr>';
	...
	if (id == null || id == undefined) {
	    for (var i=0; i<this.items.length; i++) {
			...
			if (el) {
		        ...
		        if (itm.disabled) el.style.MozOpacity = 0.2; else el.style.MozOpacity = 1;
...
function jsToolBar_itemOver(id) { 
	if (!this.box) return;
    if (document.all) { transparent = this.bgcolor; } else { transparent = 'transparent'; }
...
// Class is Loaded
if (top.jsLoader) top.jsLoader.loadFileDone('jsToolBar.js');