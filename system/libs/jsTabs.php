...
function jsTab_class(owner, id, text, toolbar, obj) {
    this.id     	= id;
    this.text   	= text;
...
function jsTabs_class(name, box) {
    // required
    this.name       = name;
    this.box        = box;
...
top.jsTab  = jsTab_class;
top.jsTabs = jsTabs_class;
...
function jsTabs_getTab(ind) {
    for (var i=0; i<this.tabs.length; i++) {
    	if (this.tabs[i] == null) continue;
    	if (this.tabs[i].id == ind || this.tabs[i] == ind) { 
    		return this.tabs[i]; 
    	}
    }
    return null;
...
function jsTabs_enable(ind) {
	tb = this.getTab(ind);
	if (tb != null) tb.disabled = false;
	this.refresh();
}

function jsTabs_disable(ind) {
	tb = this.getTab(ind);
...
function jsTabs_getTabsHTML() {
    ...
    for (i=0; i<this.tabs.length; i++) {
        ...
        tabsHTML += '<td id="tab_'+ this.name +'_'+i+'" class="'+ className +'" nowrap' +
                    '  	ondblclick = "if (top.elements[\''+ this.name +'\'].onDblClick) {top.elements[\''+ this.name +'\'].onDblClick('+i+');} "'+
...
function jsTabs_output() {
    ...
    tabsTitle = '<table cellpadding="0" cellspacing="0" class="tabs_header_tbl" width="100%">'+
				...
				'		<td class="tabs_empty" style="empty-cells: show; width: 1px; padding: 1px"><span style="font-size: 2px">&nbsp;</span></td>'+
				'		<td class="tabs_empty" style="padding-right: 5px" id="tab_'+ this.name +'_rightTitle" style="empty-cells: show; width: 1px; padding: 1px" align=right>'+ this.rightTitle +'</td>'+
				'   </tr>'+
				'</table>';
    ...
    wholeHTML +='<tr style="height: 100%"><td colspan=2 class="tabs_body"id="tab_'+ this.name +'_dsp" style="height: '+ parseInt(this.frmHeight) +'px;" valign="top">';
    if (this.frmShow) {
	    for (var i=0; i<this.tabs.length; i++) {
	    	if (this.tabs[i] == null) continue;
	        if (this.active == i) {
...
function jsTabs_resize() {
	...
	for (var i=0; i<this.tabs.length; i++) {
		if (this.tabs[i] == null) continue;
		if (this.tabs[i].obj && this.tabs[i].obj.resize) { this.tabs[i].obj.height = parseInt(tr.style.height); this.tabs[i].obj.resize(); }
	}
}
...
function jsTabs_setActive(ind) {
    ...
    if (this.active == ind) {
        ...
        if (this.tabs[ind].obj && this.tabs[ind].obj.output) {
...
// Class is Loaded
if (top.jsLoader) top.jsLoader.loadFileDone('jsTabs.js');